...
<body class="min-h-screen bg-gray-100 dark:bg-gray-800">
    ...
    <div class="container mx-auto px-5">
        <div class="flex justify-between px-2 my-3">
            <h2 class="font-bold text-3xl dark:text-white">薬袋</h2>
            <a href="{{ route('add') }}">
            <button class="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-green-400 to-blue-600 group-hover:from-green-400 group-hover:to-blue-600 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800">
              <span class="font-bold relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-800 rounded-md group-hover:bg-opacity-0">
                追加
              </span>
            </button>
          </a>
        ...
        <dl class="block sm:hidden w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <div>
            ...
            <div class="mb-14">
              ...
              <dt class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-slate-400">{{ $val->comments}}</dt>
            </div>
          @endforeach
        </dl>
        <table class="hidden sm:table w-full text-sm text-left text-gray-500 dark:text-gray-400 mb-6">
            ...
            <tbody>
              ...
              <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $val->name}}</th>
                <td class="py-4 px-6">{{ $val->amount . $val->type}}</td>
                <td class="py-4 px-6">{{ $val->days }}</td>
                <td class="py-4 px-6">{{ $val->time }}</td>
                <td class="py-4 px-6">{{ $val->comments }}</td>
                <td class="py-4 px-3">
